...
def index():
    ...
    users_seach = User.query.filter(User.name.like('%'+search_name+'%'), User.gender.like(search_gender+'%')).first()
    print(users_seach)
    ...
    return render_template('users.html', search_name=search_name, search_gender=search_gender, current_page=page, rows=rows, labels=labels, values=values, count_data=count_data, users_seach=users_seach)
